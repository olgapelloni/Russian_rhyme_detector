...
def jot_vowels_substitution(self):
    ...
    if self.transcription[-1] in self.jot_vowels.keys():
        if self.transcription[-2] not in self.all_cons and self.transcription[-2] != '\'':
...
def cons_substitutions(self):
    ...
    i = 1
    while i < transcription_length-1:
        if self.transcription[i-1] in self.pairing_cons:
            if self.transcription[i] == u'ь':
                if self.transcription[i+1] in self.jot_vowels:
                    self.transcription = ''.join(self.transcription[:i] + u'\'j' + self.transcription[i + 1:])
...
def one_vowel(self):
    v = 0
    for i in self.transcription:
        if i in self.all_vowels:
            v += 1
        if v > 1:
            break
    if (v == 1) and (u'`' not in self.transcription and u'̀' not in self.transcription):
        for i in range(0, len(self.transcription)):
...
def yo_replace(self):
    if u'ѐ' or u'е`' in self.transcription:
        possible_yo_word = self.transcription
        possible_yo_word = possible_yo_word.replace(u'ѐ', u'ё')
        possible_yo_word = possible_yo_word.replace(u'е`', u'ё')
        possible_yo_word = possible_yo_word.replace(u'̀', u'')
        possible_yo_word = possible_yo_word.replace(u'`', u'')
        # check normal form also
        morph = pymorphy2.MorphAnalyzer()
        norm_form = morph.parse(possible_yo_word)[0].normal_form
        for word in self.yo_words:
            if possible_yo_word == word or norm_form == word:
                self.transcription = self.transcription.replace(u'е`', u'ё̀')
                self.transcription = self.transcription.replace(u'ѐ', u'ё̀')
...
def after_hard_hushing(self):
    for cons in self.hard_hushing_cons:
        ...
        self.transcription = self.transcription.replace(cons + u'ьjь', cons + u'jь')
...
def post_reduction(self):
    for i in range(self.stress + 2, len(self.transcription)):
        if (self.transcription[i] == u'а' or self.transcription[i] == u'о' or self.transcription[i] == u'ы') and (self.transcription[i-1] != u'\'' and self.transcription[i-1] != u'j') or \
                (self.transcription[i-2] in self.hard_hushing_cons and self.transcription[i-1] != u'j' and self.transcription[i] == u'е'):
            self.transcription = ''.join(self.transcription[:i] + u'ъ' + self.transcription[i+1:])

        if self.transcription[i] == u'я' or self.transcription[i] == u'е' or self.transcription[i] == u'и' or \
...
def orfo_check(self):
    ...
    possible_orfo_word = possible_orfo_word.replace(u'`', u'')

    for shn_word in self.shn_words:
        if possible_orfo_word.startswith(shn_word):
